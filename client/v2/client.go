package client // import "github.com/freetsdb/freetsdb/client/v2"
...
type HTTPConfig struct {
	...
	// Username is the freetsdb username, optional.
	Username string

	// Password is the freetsdb password, optional.
	Password string

	// UserAgent is the http User Agent, defaults to "FreeTSDBClient".
	UserAgent string

	// Timeout for freetsdb writes, defaults to no timeout.
...
type Client interface {
	...
	// Write takes a BatchPoints object and writes all Points to FreeTSDB.
	Write(bp BatchPoints) error

	// Query makes an FreeTSDB Query on the database. This will fail if using
	// the UDP client.
	Query(q Query) (*Response, error)

	// QueryAsChunk makes an FreeTSDB Query on the database. This will fail if using
	// the UDP client.
...
// FreeTSDB together. BatchPoints is NOT thread-safe, you must create a separate
...
// for the query. Setting the retention policy only works on FreeTSDB versions 1.6 or greater.
...
func checkResponse(resp *http.Response) error {
	// If we lack a X-Freetsdb-Version header, then we didn't get a response from freetsdb
	...
	if resp.Header.Get("X-Freetsdb-Version") == "" && resp.StatusCode >= http.StatusInternalServerError {
		body, err := ioutil.ReadAll(resp.Body)
	...
	// If we get an unexpected content type, then it is also not from freets direct and therefore
