// Package export exports TSM files into FreeTSDB line protocol format.
package export
...
import (
	...
	"github.com/freetsdb/freetsdb/tsdb/engine/tsm1"
)

// Command represents the program execution for "freets_inspect export".
type Command struct {
...
func (cmd *Command) Run(args ...string) error {
	...
	fs.StringVar(&cmd.dataDir, "datadir", os.Getenv("HOME")+"/.freetsdb/data", "Data storage path")
	fs.StringVar(&cmd.walDir, "waldir", os.Getenv("HOME")+"/.freetsdb/wal", "WAL storage path")
	fs.StringVar(&cmd.out, "out", os.Getenv("HOME")+"/.freetsdb/export", "Destination file to export to")
	...
	fs.Usage = func() {
		fmt.Fprintf(cmd.Stdout, "Exports TSM files into FreeTSDB line protocol format.\n\n")
...
func (cmd *Command) write() error {
	...
	fmt.Fprintf(w, "# FREETSDB EXPORT: %s - %s\n", s, e)
