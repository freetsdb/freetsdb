...
// Command represents the program execution for "freets_inspect buildtsi".
type Command struct {
	Stderr  io.Writer
	Stdout  io.Writer
	Verbose bool
...
func (cmd *Command) run(dataDir, walDir string) error {
	...
	if isRoot() {
		...
		fmt.Println("if you run freets_inspect as a non-root user. You should run")
		fmt.Println("buildtsi as the same user you are running freets_inspect.")
