...
import (
	...
	"github.com/freetsdb/freetsdb/cmd/freets_tools/internal/errlist"
	"github.com/freetsdb/freetsdb/cmd/freets_tools/internal/shard"
...
func (i *importer) CreateDatabase(rp *meta.RetentionPolicyInfo) error {
	...
	nonmatchingRp := (rpi != nil) && ((rpi.Duration != rp.Duration) ||
		(rpi.ReplicaN != rp.ReplicaN) ||
	...
	if nonmatchingRp {
		...
		if _, err := i.MetaClient.CreateRetentionPolicy(i.db, rp); err != nil {
			return err
		}
	}
...
func (i *importer) createDatabaseWithRetentionPolicy(rp *meta.RetentionPolicyInfo) error {
	var err error
