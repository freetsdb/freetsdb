// Package cli contains the logic of the freets command line client.
package cli // import "github.com/freetsdb/freetsdb/cmd/freets/cli"
...
import (
	...
	"github.com/freetsdb/freetsdb/importer/v8"
...
func (c *CommandLine) Run() error {
	...
	if err := c.Connect(""); err != nil {
		msg := "Please check your connection settings and ensure 'freetsd' is running."
	...
	if len(c.ServerVersion) == 0 {
		...
		fmt.Printf("Are you sure an FreeTSDB server is listening at the given address?\n")
	} else {
	...
	if historyDir != "" {
		c.historyFilePath = filepath.Join(historyDir, ".freets_history")
...
// or executes the command as a query against FreeTSDB.
...
func (c *CommandLine) Version() {
	fmt.Println("FreeTSDB shell version:", c.ClientVersion)
	switch c.Type {
	case QueryLanguageFlux:
