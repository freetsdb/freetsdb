...
const (
	noTokenMsg = "Visit https://freetsdb.com to register for updates, FreeTSDB server management, and monitoring.\n"
)
...
func (c *CommandLine) Run() error {
	...
	if err := c.Connect(""); err != nil {
		return fmt.Errorf(
			"Failed to connect to %s\nPlease check your connection settings and ensure 'freetsd' is running.",
			c.Client.Addr())
	}
	...
	if err == nil {
		c.historyFilePath = filepath.Join(usr.HomeDir, ".freets_history")
...
func (c *CommandLine) Connect(cmd string) error {
	...
	config.UserAgent = "FreeTSDBShell/" + c.ClientVersion
...
func (c *CommandLine) help() {
	fmt.Println(`Usage:
        ...
        exit/quit/ctrl+d      quits the freets shell
...
func (c *CommandLine) Version() {
	fmt.Println("FreeTSDB shell " + c.ClientVersion)
}
