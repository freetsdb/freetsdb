...
// Ensure that HTTP responses include the FreeTSDB version.
func TestServer_HTTPResponseVersion(t *testing.T) {
	...
	got := resp.Header.Get("X-FreeTSDB-Version")
	if got != version {
...
func TestServer_Query_Tags(t *testing.T) {
	...
	writes := []string{
		...
		fmt.Sprintf("status_code,url=https://freetsdb.com value=418 %d", mustParseTime(time.RFC3339Nano, "2015-07-22T09:52:24.914395083Z").UnixNano()),
	}
	...
	test.addQueries([]*Query{
		...
		&Query{
			...
			command: `SELECT value FROM db0.rp0.status_code WHERE url !~ /https\:\/\/freetsdb\.com/`,
...
func TestServer_ContinuousQuery(t *testing.T) {
	...
	test2.addQueries([]*Query{
		...
		// TODO: restore this test once this is fixed: https://github.com/freetsdb/freetsdb/issues/3968
		&Query{
			skip:    true,
