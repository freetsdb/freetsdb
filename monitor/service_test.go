...
func TestMonitor_SetPointsWriter_StoreEnabled(t *testing.T) {
	var mc MetaClient
	mc.CreateDatabaseWithRetentionPolicyFn = func(name string, rpi *meta.RetentionPolicyInfo) (*meta.DatabaseInfo, error) {
...
func TestMonitor_StoreStatistics(t *testing.T) {
	...
	mc.CreateDatabaseWithRetentionPolicyFn = func(name string, rpi *meta.RetentionPolicyInfo) (*meta.DatabaseInfo, error) {
		...
		if got, want := rpi.Name, monitor.MonitorRetentionPolicy; got != want {
			t.Errorf("unexpected retention policy: got=%q want=%q", got, want)
		}
		if rpi.Duration != nil {
			if got, want := rpi.Duration, monitor.MonitorRetentionPolicyDuration; got != want {
				t.Errorf("unexpected duration: got=%q want=%q", got, want)
			}
		} else {
			t.Error("expected duration in retention policy info")
		}
		if rpi.ReplicaN != nil {
			if got, want := rpi.ReplicaN, monitor.MonitorRetentionPolicyReplicaN; got != want {
				t.Errorf("unexpected replica number: got=%q want=%q", got, want)
			}
		} else {
			t.Error("expected replica number in retention policy info")
		}
		return &meta.DatabaseInfo{Name: name}, nil
	}
...
func (m *MetaClient) CreateDatabaseWithRetentionPolicy(name string, rpi *meta.RetentionPolicyInfo) (*meta.DatabaseInfo, error) {
	return m.CreateDatabaseWithRetentionPolicyFn(name, rpi)
}
