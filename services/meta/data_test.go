...
import (
	...
	"github.com/freetsdb/freetsdb/pkg/testing/assert"
...
func TestData_SetPrivilege(t *testing.T) {
	...
	if got, exp := data.SetPrivilege("user1", "db1", influxql.AllPrivileges), freetsdb.ErrDatabaseNotFound("db1"); got == nil || got.Error() != exp.Error() {
		t.Fatalf("got %v, expected %v", got, exp)
	}
