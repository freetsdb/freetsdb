// Package graphite provides a service for FreeTSDB to ingest data via the graphite protocol.
package graphite // import "github.com/freetsdb/freetsdb/services/graphite"
...
func (s *Service) createInternalStorage() error {
	...
	if db := s.MetaClient.Database(s.database); db != nil {
		if rp, _ := s.MetaClient.RetentionPolicy(s.database, s.retentionPolicy); rp == nil {
			rpi := meta.RetentionPolicyInfo{Name: s.retentionPolicy}
			if _, err := s.MetaClient.CreateRetentionPolicy(s.database, &rpi); err != nil {
				return err
			}
			if err := s.MetaClient.SetDefaultRetentionPolicy(s.database, s.retentionPolicy); err != nil {
				return err
			}
